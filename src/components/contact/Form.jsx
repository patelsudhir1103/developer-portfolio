...
const commonClass =
  "input input-lg border-0 border-b-2 focus:outline-none focus:placeholder:text-picto-primary placeholder:text-[15px] md:placeholder:text-lg focus:border-picto-primary border-[#E6E8EB] dark:border-gray-700 w-full rounded-none px-0 bg-transparent text-gray-900 dark:text-white";
    ...
    <div>
      ...
      <div className="mx-2">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
          ...
          {status.error && (
            <div className="alert alert-error text-sm rounded-lg p-3 text-white mt-2">
              <span>{status.error}</span>
            </div>
          )}

          {status.success && (
            <div className="alert alert-success text-sm rounded-lg p-3 text-white mt-2">
          ...
          <button
            ...
            className="btn gap-3 max-lg:mx-auto btn-primary rounded-sm mt-5 text-[13px] md:text-[16px] w-fit font-semibold lg:mt-12.5 p-2 md:px-4 cursor-pointer disabled:bg-gray-300"
          >
            {status.submitting ? "Submitting..." : "Submit"} {telegramSVG}
          </button>
        </form>
      </div>
    </div>
